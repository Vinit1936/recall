...
async function getMaps(): Promise<{
  ...
  if (!cachedMaps) {
    const problems = (await import('../../data/gfg-problems.json')).default;
...
class GFGResolver implements PlatformResolver {
  async resolve(identifier: string): Promise<ResolveResult> {
    const raw = identifier.trim();
    if (!raw) return { found: false };

    // 1. URL pattern match (e.g. https://www.geeksforgeeks.org/problems/two-sum-1587115621/1)
    if (raw.includes('geeksforgeeks.org') || raw.includes('gfg') || raw.startsWith('http://') || raw.startsWith('https://')) {
      const match = raw.match(/geeksforgeeks\.org\/problems\/([^/]+)/i);
      const slug = match ? match[1].toLowerCase() : raw.split('/').filter(Boolean).pop()?.toLowerCase();
      ...
      if (slug) {
        ...
        const cleanTitle = slug
          .replace(/-\d+$/, '') // strip trailing numbers like -1587115621
          .replace(/\d+$/, '')  // strip trailing digits like 1416
          .replace(/-/g, ' ')
    ...
    // 2. Direct code lookup (e.g. GFG_001)
...
export const gfgResolver = new GFGResolver();
